...
标准 LoRA 可以看作由两个协同工作的部分组成。普通训练让两组使用相同的学习率；LoRA+ 保留其中一组的基础学习率，并为另一组设置更高的学习率。
...
LoRA+ 不增加网络容量，也不会补充训练数据中不存在的信息；它会改变两组参数的更新速度，以及有限训练预算下的优化轨迹。目标特征可能更早出现，重复的背景、姿势和构图也可能更早被记住。

<!-- doc-anchor: effects -->
## 与实际 LoRA 训练的关系

LoRA+ 对人物、画风、服装和一般概念采用相同的学习率机制，但不同训练目标需要观察的结果不同：
...
- **rank** 决定 LoRA 可以容纳多少变化，不直接代表学习速度。高 rank LoRA 出现欠拟合时，原因可能是学习率、训练步数或标注，而不一定需要 LoRA+。
...
LoRA+ 倍率必须和基础学习率一起理解。训练器会先确定每个训练部分的基础学习率，再将 LoRA+ 倍率应用于高学习率参数组：
...
提高整体学习率会同时加快所有 LoRA 参数。提高 LoRA+ 倍率只会加快高学习率参数组。这是 LoRA+ 在实际训练中的主要作用。

<!-- doc-anchor: ratio-guidance -->
## 倍率参数的含义

倍率表示高学习率组相对于基础组的学习率倍数，不是画质等级：

| 倍率 | 实际含义 | 使用时需要注意 |
| --- | --- | --- |
| `1.0` | 两组使用相同学习率 | 不产生 LoRA+ 的差异化学习率效果 |
| `2.0` | 高学习率组使用 2 倍学习率 | 当前训练器的默认值；这是配置行为，不代表官方通用值 |
| `4.0` | 高学习率组使用 4 倍学习率 | 需要结合基础学习率判断实际强度 |
| `8.0`～`16.0` | 高学习率组使用明显更高的学习率 | 对基础学习率、停止时机和数据重复更敏感 |

LoRA+ 论文在其实验设置中使用 `16`，sd-scripts 文档也记录了这一数值。该实验来自特定模型和任务，不能推出人物、画风或概念 LoRA 的倍率排序。当前训练器的字段默认值为 `2.0`，对应高学习率组使用基础组两倍的学习率。

<!-- doc-anchor: parameters -->
## 训练器参数

“启用 LoRA+”是当前训练器的主开关。它只控制是否向 sd-scripts 写入下列倍率参数，本身不会成为训练命令参数。

关闭开关时，界面中保留的倍率值不会进入训练配置。高级“自定义网络参数”中同名的 `loraplus_*` 项也会被忽略，以保证界面显示值和后端校验使用同一组设置。
...
仅覆盖主 UNet 的倍率。在 Anima 训练中，sd-scripts 沿用 `unet` 参数名，但它实际对应主要的 DiT 网络。
...
未训练文本编码器、启用“仅训练 UNet”，或缓存设置使文本编码器不参与训练时，这个参数没有作用。提高文本编码器的倍率可能让触发词更早产生明确响应，也可能让模型更早依赖该固定触发词，降低其他提示词的控制能力。
...
全局倍率为空时，可以只填写一个组件倍率，使 LoRA+ 只作用于该训练部分。启用 LoRA+ 时至少需要填写一个倍率；某个组件的专用倍率和全局倍率都为空时，该组件不会应用 LoRA+。

<!-- doc-anchor: good-cases -->
## LoRA+ 改变的可观察量

LoRA+ 仅提高高学习率参数组的有效学习率，因此会改变以下量：

- 相同步数时，高学习率组累计的参数更新幅度。
...
在以下数据条件下，提高倍率会让重复证据更快累积：

- 数据量很小，或多张图片共享相同背景、姿势和构图。
- 图片文件很多，但包含连续帧、重复裁剪或高度相似的图。
- 基础学习率已经较高，乘以倍率后的实际学习率可能过大。
- 数据中反复出现伙伴角色、水印、特效或未被正确标注的元素。
- 当前训练已经容易出现构图僵化、背景绑定或提示词响应下降。
- 优化器在内部管理学习率，训练中的实际倍率可能发生变化。

LoRA+ 不能改变模型容量、补充训练数据或修正标注，也不会替你确定应在第几步停止训练。倍率增大时，目标特征与重复证据都可能在更早的 checkpoint 中出现。
...
当数据集、训练 seed、rank、alpha、基础学习率、优化器、调度器、总步数、生成 seed 和提示词保持一致时，差异才可能归因于 LoRA+ 开关或倍率。
...
如果较高倍率只让相近结果更早出现，说明变化主要体现在学习速度上。如果背景绑定、构图重复或提示词控制下降也更早出现，则观测到的是倍率、基础学习率、数据重复和停止时机共同作用的结果。

loss 可以帮助发现训练异常和趋势，但不能单独判断人物还原度、画风迁移或提示词控制能力。
...
<div class="doc-equation" role="group" aria-label="标准 LoRA 权重增量公式">
  ...
  <p><span class="doc-math-var">A</span> 对应 <code>lora_down</code>，<span class="doc-math-var">B</span> 对应 <code>lora_up</code>；<span class="doc-math-var">r</span> 表示 rank，<span class="doc-math-var">α</span> 表示 alpha。</p>
</div>
...
| Prodigy、ProdigyPlusScheduleFree | 当前训练器不支持该组合 | D-adaptation 约束与当前参数组处理不能可靠保留 LoRA+ 的独立组学习率，界面与后端都会阻止该组合。 |
...
当前训练器使用的标准外部 scheduler 按相同比例缩放已注册参数组，因此会保留初始 LoRA+ 倍率关系。warmup 控制训练早期的整体学习率变化，不等同于 LoRA+ 倍率。对于内部自适应优化器或自定义多组配置，参数组日志是可观察的契约。

<!-- doc-anchor: support -->
## 支持范围

当前训练器只为 sd-scripts 已实现 LoRA+ 参数分组的原生网络模块提供开关：
...
`lycoris.kohya` 不显示这个开关，避免向当前模块传入未经确认的参数。LoHa 和 LoKr 的支持表示 sd-scripts 能为所列参数设置更高的学习率，不表示标准 LoRA+ 论文对这些分解方式给出了相同的实验结论。

<!-- doc-anchor: tensorboard -->
## TensorBoard 记录

启用 LoRA+ 后，sd-scripts 会分别记录基础组和高学习率组。标准 SDXL LoRA 记录以下名称：
...
名称中的 `plus` 表示高学习率参数组。使用分块学习率或其他多组参数配置时，名称和曲线数量还可能增加。

训练器会记录 optimizer 暴露的参数组学习率。该数值不一定等于内部自适应 optimizer 对每个参数施加的真实更新尺度；对常规优化器，可以通过两条曲线检查组间倍率关系。
...
- [sd-scripts 高级网络训练说明（固定版本）](https://github.com/kohya-ss/sd-scripts/blob/37a1cbbc5725ed2a3575506e7bd2001c9908ac92/docs/train_network_advanced.md)
- [sd-scripts LoHa/LoKr 说明（固定版本）](https://github.com/kohya-ss/sd-scripts/blob/37a1cbbc5725ed2a3575506e7bd2001c9908ac92/docs/loha_lokr.md)

当前训练器的组合限制、自动关闭和有效学习率边界由字段注册表、校验器、适配器与测试确定。LoHa/LoKr 表格描述的是 sd-scripts 的扩展映射，不代表 LoRA+ 论文验证了相同效果。
