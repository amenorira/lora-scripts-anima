...
因此，LoRA+ 主要影响目标特征在训练中出现得有多快，而不是模型能够学习哪些内容。训练数据和标注决定模型会反复学习哪些内容。LoRA+ 可能让目标特征更早出现，也可能让重复的背景、姿势和构图更早被记住。
...
| `2.0` | 高学习率组使用 2 倍学习率 | 本训练器的默认值，差异相对温和 |
| `4.0` | 高学习率组使用 4 倍学习率 | 需要结合基础学习率判断实际强度 |
| `8.0`～`16.0` | 高学习率组使用明显更高的学习率 | 对基础学习率、停止时机和数据重复更敏感 |

LoRA+ 论文在其实验设置中使用 `16`，sd-scripts 文档也记录了这一数值。该实验来自特定模型和任务，不能推出人物、画风或概念 LoRA 的倍率排序。本训练器的字段默认值为 `2.0`，对应高学习率组使用基础组两倍的学习率。

<!-- doc-anchor: parameters -->
## 训练器参数

“启用 LoRA+”是本训练器的主开关。它只控制是否向 sd-scripts 写入下列倍率参数，本身不会成为训练命令参数。
