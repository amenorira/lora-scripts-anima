...
图片数量不会直接决定是否适合 LoRA+。更重要的是有效独立图片数和内容多样性：
...
- **基础学习率**决定倍率相乘前的起点。同一个 `2.0` 倍率，在 `1e-4` 和 `2e-4` 基础学习率下代表不同的实际训练强度。
...
LoRA+ 论文将 `16` 作为其实验设置中的推荐倍率，sd-scripts 文档也转述了这一数值。该结论来自特定模型和任务，不能直接视为人物、画风或概念 LoRA 的通用推荐值。本训练器默认使用 `2.0`，提供较温和的初始差异。
...
| 训练部分 | 使用的倍率 |
| --- | --- |
| UNet/DiT | 优先使用 `loraplus_unet_lr_ratio`，为空时使用 `loraplus_lr_ratio` |
| 文本编码器 | 优先使用 `loraplus_text_encoder_lr_ratio`，为空时使用 `loraplus_lr_ratio` |
...
LoRA+ 是否有价值，需要通过当前训练结果判断。以下现象通常更具有参考意义：

- 在预期步数内，人物身份、画风或目标概念仍然明显不足。
- 提高整体学习率会破坏细节或造成不稳定，但希望只加快一部分 LoRA 参数。
- 训练时间或总步数有限，需要比较相同步数下能否更早得到可用结果。
- 已经有一组未启用 LoRA+ 的结果，可以作为学习速度和最终质量的对照。

如果普通 LoRA 已经在预期步数内达到稳定结果，LoRA+ 的额外收益通常有限。仅仅因为训练器提供这一选项，并不意味着当前配置需要启用它。
...
LoRA+ 不能解决模型容量不足、训练数据缺失或标注错误。它也不会自动给出新的停止步数。倍率越高，最佳结果和过拟合都可能更早出现，因此原有训练终点不一定仍然适用。
...
| 每组配置中表现最好的 checkpoint | LoRA+ 是否改善了实际得到的最佳结果 |

有可比性的实验通常保持数据集、训练 seed、rank、alpha、基础学习率、优化器、调度器和总步数一致，只改变 LoRA+ 开关或一个倍率。预览使用相同生成 seed 和提示词，可以减少随机结果对判断的干扰。
...
如果较高倍率只让最佳结果更早出现，而各配置的最佳质量相近，它的主要作用是缩短达到该结果所需的训练步数。如果背景绑定、构图重复或提示词控制下降也更早出现，则需要同时评估倍率、基础学习率、数据重复和停止时机。
...
标准 LoRA 通常让两组参数使用相同学习率。LoRA+ 保留 `lora_down` 的基础学习率，并提高 `lora_up` 的学习率：
...
使用常规学习率调度器时，各参数组通常按相同比例变化，初始 LoRA+ 倍率关系会保留。Warmup 控制训练早期的整体学习率变化，不等同于 LoRA+ 倍率。Schedule-Free、Automagic3 等内部动态优化器应以训练日志中的实际曲线为准。
