...
优化器决定模型怎样根据梯度更新参数。实际选择主要看收敛过程、显存开销和稳定性，而不是给优化器排一个通用画质名次。

没有已有配置时，可用本项目提供的 AdamW8bit 初始配置建立对照。更换优化器时，要同时检查学习率的含义：相同数值在不同优化器中不一定代表相同更新幅度。
...
优化器处理梯度，不会自动识别错误标注、低质量图片或目标人物。遇到这些问题仍需处理数据，但这不意味着优化器对训练结果没有影响。
...
学习率控制一次更新的幅度。过低时，目标特征可能迟迟不明显；过高时，可能更快拟合训练图，也可能产生损失尖峰、构图固定或提示词响应下降。

下表中的数值是本项目的初始配置，不是跨数据集的最优值。普通 AdamW、Muon、LoRA-Muon 与自动步长优化器对更新的处理不同，不能仅凭学习率数字比较训练强弱。
...
动量系数的具体含义取决于优化器，不能都解释为 AdamW 的两项统计。训练已经稳定时，不必仅为了追求还原度而逐项修改这些参数。
...
- **学习率缩放**（`adjust_lr_fn`，默认 `match_rms_adamw`）：`match_rms_adamw` 提供便于从 AdamW 学习率开始比较的量级，不保证每一步的更新与 AdamW 相同。改为其他规则后，应重新检查学习率。
...
- **迭代次数**（`ns_steps`，默认 `5`）：增加次数会增加计算量，但当前多项式系数不保证迭代越多就越接近精确正交化。日常训练可以保留默认值；需要比较计算开销时，再单独测试迭代次数。
...
理论上的表示不变性有满秩等前提，实际实现还使用正则项与有限次迭代。它的结构动机不等于已证明更适合所有 Anima 人物或画风训练。
...
Adan 除了跟踪梯度，还跟踪相邻步骤的梯度变化，用这些信息调整更新方向。它可以作为收敛速度的对照方案，但同一学习率下，不能普遍断言它的实际步长一定大于 AdamW。

使用本项目提供的初始学习率建立对照，再按目标特征出现的速度、损失稳定性和最佳模型结果调整。不要把其他任务使用的高学习率直接当作少图 LoRA 的推荐值。
...
这里的 `alpha` 是优化器的混合权重，与网络 Alpha 不是同一参数。对于步数较少的训练，重点观察长期状态是否来得及发挥作用，而不是仅因有“长期记忆”就认为一定更好。
...
| 参数 | 实际作用 | 使用时关注 |
| --- | --- | --- |
| `max_precondition_dim` | 决定多长的矩阵轴参与矩阵预条件处理 | 提高上限会增加统计矩阵与坐标矩阵的显存、计算开销 |
| `precondition_frequency` | 决定多久重算一次坐标系 | 间隔大时开销较低，但坐标更新不够及时 |
| `shampoo_beta` | 决定预条件统计保留多少历史 | 较大时更平滑，也更慢响应近期变化 |
| `normalize_gradient` | 用更新张量的均方根归一化整体尺度 | 不是挑出某些方向单独压制 |
| `correct_bias` | 修正历史平均从零开始造成的偏差 | 不等于学习率预热 |

较短的矩阵轴更容易在当前维度上限内得到完整预条件处理，这解释了不同 LoRA/LoKr 结构的资源差异；它不能单独证明哪种结构训练效果更好。
...
| 数值 | 含义 | 不能怎样理解 |
| --- | --- | --- |
| `D` | 优化器根据训练过程估计的尺度 | 不是一个不经过其他处理的最终更新量 |
| 学习率字段 | 参与步长计算的倍率 | 不能直接与 AdamW 的数值比较 |
| `d0` | 初始步长估计 | 不是始终固定的学习率 |
| `d_coef` | 调整步长估计的尺度 | 不是最终参数变化的严格线性倍数 |

本项目的 `d0` 默认值为 `1e-6`，`d_coef` 默认值为 `1.0`。

ProdigyPlusScheduleFree 还维护供评估与保存使用的平均权重。平均可以减少对最后一个训练状态的依赖，但也会使刚学到的变化逐步反映到保存结果中。

`schedulefree_c` 调整平均权重对近期结果的侧重。正数越大，近期更新通常占比越高；`0` 使用原有规则，不能理解为平均最平滑。它调整的是平均响应，不是固定的画质强度。

日志中的参数组学习率、`D × lr` 与真正的参数更新并不是同一指标。查看曲线时应先确认记录的是什么，再结合多个训练阶段的生成结果判断学习是否充分。

- **D 增长限制**（`d_limiter`，默认开启）：开启时 `D` 的估计值每步最多上升约两成，训练初期不稳定的梯度不会把估计值单步推高，上升速度随之变慢。曲线上升过慢时可以关闭；关闭后单次高估会直接进入学习率。该项对 SPEED 无效，SPEED 自带涨幅限制。
- **停止调整 D 的步数**（`prodigy_steps`，默认 `0`）：到达该步数后 `D` 冻结在当前值，估计缓存同时释放。适用于 `D` 已确认合适、后半程不再变化的场景；`0` 表示全程持续估计。
- **偏差校正与自动预热**（`use_bias_correction`，默认关闭）：改用 RAdam 变体，结合偏差校正与自动预热。会显著拖慢 `D` 的调整，上游标注至多慢十倍，可配合 SPEED 缓解；`D` 曲线长期不动时优先检查这一项。
- **SPEED 估计器**（`use_speed`，默认关闭）：换用另一种估计方式。标准方式逐步累计梯度与位移的相关性；SPEED 只在方向性进展超过历史峰值时上调 `D`，状态占用更少、对梯度整体缩放不敏感。上游认为同时训练多个网络时它可能是更好的选择。上游标注为高度实验性，与权重衰减同用时可能不稳定。
- **谨慎更新**（`use_cautious`，默认关闭）：按当前梯度方向筛除不一致的更新分量。该优化器没有一阶动量，上游把掩码直接作用在更新上，与论文有偏差，效果可能有限。
- **正交梯度更新**（`use_orthograd`，默认关闭）：只使用与当前权重方向正交的梯度分量更新参数，上游称有助于抑制过拟合、改善泛化。

- **按参数组分别估计 D**（`split_groups`，默认开启）：本训练器将文本编码器与 DiT 分为不同参数组，两者的梯度分布差别明显。开启时各组分别估计 `D`，力度互不影响；关闭后所有组共用一个由全部梯度混合估计的 `D`，即原版 Prodigy 的行为，通常仅在对照参考实现时关闭。
- **多组共用平均 D**（`split_groups_mean`，默认关闭）：仅在上一项开启时有作用。开启后各组仍分别估计，但实际使用所有组 `D` 的调和平均（偏向最小的一组），组间实际力度的比例更贴近所设学习率的比例，适用于某一组的 `D` 长期不上升、限制其余组的情况。关闭时各组完全独立，实际力度随各自的梯度情况分化。
- **分解二阶矩**（`factored`，默认开启）：二阶矩统计通常与参数同尺寸，开启后改用行、列两个统计量近似，显存占用下降，代价是近似误差。出现 NaN 或 `D` 长期不上升时可以关闭。LoRA 的可训练参数量小，本项在本训练器中收益有限。
- **分解统计保持 FP32**（`factored_fp32`，默认开启）：分解近似对精度敏感，这份统计固定以 FP32 保存，避免半精度进一步放大误差。只有梯度以半精度保存时，关闭本项才能减少显存占用。
- **权重衰减跟随学习率**（`weight_decay_by_lr`，默认开启）：开启时每步衰减量为权重衰减 × `D`，随 `D` 从 `1e-6` 量级逐步增大；关闭后从第一步起即为全额绝对衰减。训练初期参数每步的移动量为 `1e-6` 量级，`0.01` 的衰减比参数更新大几个数量级，会将权重持续向 0 收缩。上游为此标注“不清楚就不要改”，保持默认。
- **内部更新缩放**（`use_stableadamw`，默认开启）、**数值稳定项**（`eps`）：见前面的对应小节。开启更新缩放，或把 `eps` 填成 `None`（改用 Adam-atan2）时，界面会锁定全局梯度裁剪为 `0`。自适应学习率始终不增长或被高估时，上游建议关闭更新缩放。
- **实验与进阶选项**（界面未提供控件，可通过自定义 `optimizer_args` 传入）：`use_grams` 与 `use_adopt` 为实验性更新变体，上游自评效果可能有限；`use_focus` 处理大步长下的噪声，传入后会自动关闭 `factored` 与 Adam-atan2；`beta3` 调整 `D` 估计的滑动平均系数，默认随 β2；`stochastic_rounding` 仅对 BF16 权重有意义，FP32 参数下无效。
...
同样，批次大小与梯度累积不能只看乘积。完整累积窗口的有效批次为“批次大小 × 累积步数 × GPU 数”，但分桶尾批、实际抽样组合和浮点计算等因素仍可能造成差异。
...
3. Anima 配方对照使用上方学习率主表中对应的工程起点。对比优化器时，先分别找到可用的学习率范围，再固定数据、训练预算和生成评估条件。相同学习率的实验可以作为额外对照，但不代表实际更新幅度相同，也不自动构成更公平的比较。
