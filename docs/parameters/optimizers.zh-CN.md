...
ProdigyPlusScheduleFree 的学习率不来自输入值。优化器在训练中持续估计一个名为 `D` 的步长尺度，可以理解为权重当前离开初始位置的距离，每步实际以 `D × 1.0` 作为学习率；界面上填的学习率固定为 `1.0`，只作缩放因子。`D` 从 `d0` 起步，只增不减。

Schedule-Free 部分为权重维护一份滑动平均，保存的 LoRA 是这份平均，不是最后一步的原始权重。这是外部调度器固定为 `constant` 的原因：力度的变化全部由 `D` 承担。平均使成品对训练停在第几步不敏感，但早期较弱的训练步也会保留在平均中。

训练力度通过 TensorBoard 曲线观察：`lr/d*lr/unet` 与 `lr/d*lr/textencoder` 分别记录两个组件的 `D`（曲线值等于 `D` 乘以学习率，学习率为 `1.0` 时即 `D` 本身）。曲线长期停留在低位说明估计值未上升、训练力度不足；上升后趋于平稳说明估计收敛。监控页的学习率读数来自外部调度器，在该优化器下恒为 `1.0`，不反映实际力度。

- **初始 D 估计值**（`d0`，默认 `1e-6`）：`D` 的起点，也是下限。通常无需调整，估计器会自行抬升 `D`；曲线长期处于低位、训练力度不足时，设为 `1e-5`（必要时 `1e-4`）可省去估计器最初的上升过程。
- **D 倍数**（`d_coef`，默认 `1.0`）：对估计出的 `D` 整体乘的系数，作用与修改学习率相同（`2.0` 约为力度翻倍）。学习率字段锁定后，整体放大或缩小力度通过它完成，适用于 `D` 曲线已平稳但整体偏弱或偏强的情况。
...
- **平均窗口响应速度**（`schedulefree_c`，默认 `0`）：决定成品平均偏向多近的历史。数值越大，最近的训练步在平均中占比越高，成品越接近最近的状态；数值越小，整段训练的权重越平均。`0` 沿用原始规则，在默认 β1=0.9 下与 `10` 等效：低于 `10` 比默认更平滑，高于 `10` 比默认更偏向近期。上游参考值：短训练、小 batch 从 `6` 到 `12` 起步，长训练或大 batch 可用到 `50` 以上。训练正常但成品质量偏差时，优先调整这一项。
