...
| `lycoris.kohya` (LoCon / algo `lora` only) | `lora_up` et al. | The LyCORIS adapter groups by the parameter name `lora_up`; the other LyCORIS algorithms (LoHa/LoKr/GLoRA, …) do not match that grouping, so the ratio has no effect |
